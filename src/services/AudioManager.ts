...
export class AudioService {
  ...
  private _processorNode: AudioWorkletNode | ScriptProcessorNode | null = null;
  ...
  private constructor() {
    ...
    this._useAudioWorklet = !!this._audioContext.audioWorklet;
    if (!this._useAudioWorklet) {
      console.warn("[AudioManager] AudioWorklet is not supported in this browser. Using ScriptProcessorNode instead.");
    }
  }
  ...
  public playAudio = async () => {
    ...
    try {
      ...
      // 保存当前节点的引用，避免闭包中的引用问题
      const currentNode = this._sourceNode;
      this._sourceNode.onended = () => {
        if (currentNode && currentNode === this._sourceNode) {
          try {
  ...
  private loadAudioWorklet = async () => {
    ...
    if (this._useAudioWorklet) {
      ...
      console.log("[AudioManager][prepareMediaResources] Using ScriptProcessorNode instead of AudioWorklet.")
    }
  }
  ...
  private initProcessorNode = () => {
    ...
    if (this._useAudioWorklet) {
      ...
      (this._processorNode as AudioWorkletNode).port.onmessage = (e: MessageEvent) => {
      ...
      // 使用ScriptProcessorNode作为替代方案
      this._processorNode = this._audioContext.createScriptProcessor(960, 1, 1);
      
      // 处理节点的回调函数，用于处理音频数据
      (this._processorNode as ScriptProcessorNode).onaudioprocess = (e: AudioProcessingEvent) => {
        const inputData = e.inputBuffer.getChannelData(0);
        const audioLevel = this.detectAudioLevel(inputData);
        // console.log("[AudioManager][processorNode.onaudioprocess] Audio Level:", audioLevel.toFixed(2));
        this._onProcess?.(audioLevel, inputData);
      };
    }
  }
  ...
  public clearProcessorNode = () => {
    if (this._processorNode) {
      if (this._useAudioWorklet && this._processorNode instanceof AudioWorkletNode) {
        (this._processorNode as AudioWorkletNode).port.onmessage = null;
      } else if (!this._useAudioWorklet && this._processorNode instanceof ScriptProcessorNode) {
        (this._processorNode as ScriptProcessorNode).onaudioprocess = null;
      }
